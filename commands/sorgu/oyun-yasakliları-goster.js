...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('oyun-yasaklilari-goster')
    .setDescription('Kamptaki aktif oyun yasaklı kullanıcıları listeler.')
    .addStringOption(opt => 
  ...
  async execute(interaction) {
    await interaction.reply('Oyun yasaklıları listesi yükleniyor...');
    ...
    const yasaklar = await Yasak.find({ kampId: kamp._id, tur: 'oyun', aktif: true });

    if (!yasaklar.length) {
      return interaction.editReply(`**${kamp.isim}** kampında aktif oyun yasaklısı kimse bulunmuyor.`);
    }
    ...
    const embed = new EmbedBuilder()
      .setColor(0xFFA500)
      .setTitle(`${kamp.isim} - Aktif Oyun Yasaklıları`)
      .setDescription(liste.length > 4096 ? liste.substring(0, 4093) + '...' : liste)
      .setTimestamp()
      .setFooter({ text: `Toplam ${yasaklar.length} aktif oyun yasaklısı listeleniyor.` });
