...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('ittifak-yasaklilari-goster')
    .setDescription('Kamptaki (veya genel ittifak kapsamında) aktif ittifak yasaklılarını listeler.')
    .addStringOption(opt => 
  ...
  async execute(interaction) {
    await interaction.reply('İttifak yasaklıları listesi yükleniyor...');
    ...
    const yasaklar = await Yasak.find({ kampId: kamp._id, tur: 'ittifak', aktif: true });

    if (!yasaklar.length) {
      return interaction.editReply(`**${kamp.isim}** kampında aktif ittifak yasaklısı kimse bulunmuyor.`);
    }
    ...
    const embed = new EmbedBuilder()
      .setColor(0x800080)
      .setTitle(`${kamp.isim} - Aktif İttifak Yasaklıları`)
      .setDescription(liste.length > 4096 ? liste.substring(0, 4093) + '...' : liste)
      .setTimestamp()
      .setFooter({ text: `Toplam ${yasaklar.length} aktif ittifak yasaklısı listeleniyor.` });
